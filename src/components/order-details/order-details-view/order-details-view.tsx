...
export default function OrderDetailsView({
  ...
  const ingredientsFromStore: Array<TIngredient> =
  ...
  const [uniqueIngredientsCount, setUniqueIngredientsCount] =
    useState<TMonoTypeObject<number> | null>(null);

  useEffect(() => {
    const ingredients: Array<TIngredient> = [];
    const uniqueIngredientsCount = order.ingredients.reduce(
    ...
    Object.keys(uniqueIngredientsCount).forEach((ingredientId) => {
      const ingredientForAdd = ingredientsFromStore.find(
  ...
  return (
    <article className={styles.wrapper}>
      <p className="text text_type_digits-default mb-8">{number}</p>
      <h3 className="text text_type_main-medium mb-1">{order.name}</h3>
              ...
              <div className={`${styles.countAndPrice}`}>
                <p className="text text_type_digits-default mr-1">
                  {uniqueIngredientsCount &&
                    uniqueIngredientsCount[ingredient._id] +
                      ' x ' +
                      ingredient.price}
                </p>
